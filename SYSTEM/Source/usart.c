...
uint8_t USART2_IRQHandler(void) // 串口2用于蓝牙通信
{
	if (USART_GetITStatus(USART2, USART_IT_RXNE) == SET)
	{
		Serial2_RxData = USART_ReceiveData(USART2);
		if (Serial2_RxData == 0x2C && flag == 0) // 接收帧头
		{
			flag = 1;
			openmv_2[0] = 0x2C;
		}
		else if (Serial2_RxData == 0x12 && flag == 1) // 二次验证帧头
		{
			flag = 2;
			openmv_2[1] = 0x12;
		}
		...
		{
			flag = 3;
			openmv_2[2] = Serial2_RxData;
		}
		else if (flag == 3)
		{
			flag = 4;
			openmv_2[3] = Serial2_RxData;
		}
		else if (flag == 4)
		{
			flag = 5;
			openmv_2[4] = Serial2_RxData;
		}
		else if (flag == 5)
		{
			flag = 6;
			openmv_2[5] = Serial2_RxData;
		} // 到此接收数据结束

		else if (flag == 6)
		{
			if (Serial2_RxData == 0x5B) // 判断是否为帧尾
			{
				flag = 0;
				openmv_2[6] = 0x5B;
			}
			else if (Serial2_RxData != 0x5B) // 不是帧尾 清除数据 重新来过
			{
				flag = 0;
				for (i = 0; i < 7; i++)
				{
